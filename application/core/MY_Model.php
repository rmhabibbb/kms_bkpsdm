...
class MY_Model extends CI_Model
{
	...
	public function get_by_order_any_limit($ref, $order,$tm,$mas, $cond = '')
	{		

		$this->db->select('TM , MAS , ((TM-'.$tm.')*(TM-'.$tm.') + (MAS-'.$mas.')*(MAS-'.$mas.')) as kj');
	...
	public function getDataLike2($x, $cond = '')
	{

		 
			$this->db->select('*')
		        ->group_start()
		              ->or_like('judul', $x)->or_like('pengetahuan', $x)
		        ->group_end()
		        ->where($cond);
	...
	public function cek_password_length($password)
        ...
        public function cek_passwords($password,$confirm_password){
            ...
            $output .= '<i style="color:red">Password tidak sama !</i>';
          }
          return $output;
        } 
